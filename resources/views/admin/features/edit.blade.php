    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.features.update", [$feature->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('make') ? 'is-invalid' : '' }}" name="make_id" id="make_id" required>
                    @foreach($makes as $id => $make)
                        <option value="{{ $id }}" {{ (old('make_id') ? old('make_id') : $feature->make->id ?? '') == $id ? 'selected' : '' }}>{{ $make }}</option>
                    @endforeach
                </select>
                @if($errors->has('make'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('modele') ? 'is-invalid' : '' }}" name="modele_id" id="modele_id" required>
                    @foreach($modeles as $id => $modele)
                        <option value="{{ $id }}" {{ (old('modele_id') ? old('modele_id') : $feature->modele->id ?? '') == $id ? 'selected' : '' }}>{{ $modele }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name" id="name" required>{{ old('name', $feature->name) }}</textarea>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $feature->description) !!}</textarea>
