    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.colors.update", [$color->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $color->code) }}">
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('url_image') ? 'is-invalid' : '' }}" type="text" name="url_image" id="url_image" value="{{ old('url_image', $color->url_image) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('make') ? 'is-invalid' : '' }}" name="make_id" id="make_id">
                    @foreach($makes as $id => $make)
                        <option value="{{ $id }}" {{ (old('make_id') ? old('make_id') : $color->make->id ?? '') == $id ? 'selected' : '' }}>{{ $make }}</option>
                    @endforeach
                </select>
                @if($errors->has('make'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('modele') ? 'is-invalid' : '' }}" name="modele_id" id="modele_id">
                    @foreach($modeles as $id => $modele)
                        <option value="{{ $id }}" {{ (old('modele_id') ? old('modele_id') : $color->modele->id ?? '') == $id ? 'selected' : '' }}>{{ $modele }}</option>
                    @endforeach
                </select>
