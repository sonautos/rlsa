...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.truck.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.trucks.update", [$truck->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('supplier') ? 'is-invalid' : '' }}" name="supplier_id" id="supplier_id">
                    @foreach($suppliers as $id => $supplier)
                        <option value="{{ $id }}" {{ (old('supplier_id') ? old('supplier_id') : $truck->supplier->id ?? '') == $id ? 'selected' : '' }}>{{ $supplier }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('plates') ? 'is-invalid' : '' }}" type="text" name="plates" id="plates" value="{{ old('plates', $truck->plates) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('chauffeur') ? 'is-invalid' : '' }}" type="text" name="chauffeur" id="chauffeur" value="{{ old('chauffeur', $truck->chauffeur) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $truck->price) }}" step="0.01">
                @if($errors->has('price'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_load') ? 'is-invalid' : '' }}" type="text" name="date_load" id="date_load" value="{{ old('date_load', $truck->date_load) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_cmr') ? 'is-invalid' : '' }}" type="text" name="date_cmr" id="date_cmr" value="{{ old('date_cmr', $truck->date_cmr) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="number" name="status" id="status" value="{{ old('status', $truck->status) }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('paid') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="paid" id="paid" value="1" {{ $truck->paid || old('paid', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $truck->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shippment') ? 'is-invalid' : '' }}" name="shippment_id" id="shippment_id">
                    @foreach($shippments as $id => $shippment)
                        <option value="{{ $id }}" {{ (old('shippment_id') ? old('shippment_id') : $truck->shippment->id ?? '') == $id ? 'selected' : '' }}>{{ $shippment }}</option>
                    @endforeach
                </select>
