...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.shippmentslist.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.shippmentslists.update", [$shippmentslist->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="ref">{{ trans('cruds.shippmentslist.fields.ref') }}</label>
                <input class="form-control {{ $errors->has('ref') ? 'is-invalid' : '' }}" type="text" name="ref" id="ref" value="{{ old('ref', $shippmentslist->ref) }}" required>
                @if($errors->has('ref'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('entity') ? 'is-invalid' : '' }}" name="entity_id" id="entity_id" required>
                    @foreach($entities as $id => $entity)
                        <option value="{{ $id }}" {{ (old('entity_id') ? old('entity_id') : $shippmentslist->entity->id ?? '') == $id ? 'selected' : '' }}>{{ $entity }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $shippmentslist->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_delivery') ? 'is-invalid' : '' }}" type="text" name="date_delivery" id="date_delivery" value="{{ old('date_delivery', $shippmentslist->date_delivery) }}" required>
